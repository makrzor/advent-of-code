...
for i in range(1, YEARS_CHECK):
    gather_neighbour_info()
    change_fields()
outcomes = [-1] * YEARS_CHECK
for i in range(YEARS_CHECK, 2 * YEARS_CHECK):
    gather_neighbour_info()
    change_fields()
    outcomes.append(count(2) * count(3))
for j in range(2, 100):
    if outcomes[-j] == outcomes[-1]:
        cycle = j - 1
        rest = YEARS_MAX % cycle
        base = cycle * (len(outcomes) / cycle)
        if base + rest >= len(outcomes):
            base -= cycle
        print(outcomes[base + rest])
        exit(0)
print("YEARS_CHECK too low, please increase")
exit(1)
