...
dimensions_prev = dimensions
dimensions = last_column - first_column + bottom_line - top_line
grid = [[" " for row in range(bottom_line - top_line + 1)] for col in range(last_column - first_column + 1)]
for i in range(size):
    grid[position[i][0] - first_column][position[i][1] - top_line] = "#"
for row in range(bottom_line - top_line + 1):
    for col in range(last_column - first_column + 1):
