...
achievable_with_y_initial_speeds = []
for x_initial_speed in range(min_speed_for_distance, ranges["x"][1] + 1):
    ...
    for steps in achievable_in_steps:
        for y_initial_speed in range(steps // 2):
            step = 2 * y_initial_speed + 1
            y_distance = 0
            y_speed = -y_initial_speed - 1
            while step < steps:
                y_distance += y_speed
                y_speed -= 1
                step += 1
            ...
            if ranges["y"][0] <= y_distance <= ranges["y"][1]:
                if y_initial_speed not in achievable_with_y_initial_speeds:
                    achievable_with_y_initial_speeds.append(y_initial_speed)

max_y_initial_speed = max(achievable_with_y_initial_speeds)
print(max_y_initial_speed * (max_y_initial_speed + 1) // 2)
