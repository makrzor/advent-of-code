...
LIST = "oaoe rxeq vssdqtu xrk cjv yaoqp loo,\
    ...
    for i in range(len(words)):
        ...
        for j in range(i + 1, len(words)):
            ...
            for k in range(len(word_1)):
                position = word_2.find(word_1[k])
                if position == -1:
                    break
                word_2 = word_2[:position] + word_2[position + 1:]
                if word_2 == "":
                    return(0)
    return(1)

for row in LIST.split(','):
    valid_count += iterate(row.split())
    
#print valid_count
print valid_count + 1 # why???
