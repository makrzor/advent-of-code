...
INSTRUCTIONS_LIST = "g dec 231 if bfx > -10,\
k dec -567 if wfk == 0,\
jq inc 880 if a < 2,\
sh inc -828 if nkr < -5,\
w inc 595 if nkr > -10,\
...
for instruction in INSTRUCTIONS_LIST.split(','):
    ...
    for register in parameters[0], parameters[4]:
        if register not in registers.keys():
            registers[register] = 0
    if parameters[5] == '>' and registers[parameters[4]] > int(parameters[6]) \
       or parameters[5] == '<' and registers[parameters[4]] < int(parameters[6]) \
       or parameters[5] == '>=' and registers[parameters[4]] >= int(parameters[6]) \
       or parameters[5] == '<=' and registers[parameters[4]] <= int(parameters[6]) \
       or parameters[5] == '==' and registers[parameters[4]] == int(parameters[6]) \
       or parameters[5] == '!=' and registers[parameters[4]] != int(parameters[6]):
        if parameters[1] == 'inc':
            registers[parameters[0]] += int(parameters[2])
        elif parameters[1] == 'dec':
            registers[parameters[0]] -= int(parameters[2])
        peak = max(registers.values())
        if peak > highest_value:
            highest_value = peak
print(highest_value)
