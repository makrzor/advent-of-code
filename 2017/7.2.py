...
def weigh_program(index, weights, tower_indexes):
    program_weight = weights[index]
    for i in range(1, len(tower_indexes[index])):
        program_weight += weigh_program(tower_indexes[index][i], weights, tower_indexes)
    return program_weight

def check_tower_weights(index, tower_weights, tower_indexes):
    weight = tower_weights[tower_indexes[index][1]]
    for i in range(2, len(tower_indexes[index])):
        if weight != tower_weights[tower_indexes[index][i]]:
            return 1
    return 0
...
program_weights = []
program_subs = []
program_tower_indexes = []
program_tower_weights = []
program_tower_errors = [0] * len(structure_split)
for i in range(len(structure_split)):
    program_data = structure_split[i].split()
    program_names.append(program_data[0])
    program_weights.append(int(program_data[1].split(')')[0][1:]))
    for j in range(3, len(program_data)):
        program_subs.append(program_data[j][:-1])
for i in range(len(program_names)):
    if program_names[i] not in program_subs:
        base = i
        break
print(base, program_names[base])
for i in range(len(structure_split)):
    program_data = structure_split[i].split()
    program_tower_indexes.append([-1])
    for j in range(3, len(program_data)):
        program_tower_indexes[i].append(program_names.index(program_data[j][:-1]))
for i in range(len(program_names)):
    program_tower_weights.append(weigh_program(i, program_weights, program_tower_indexes))
for i in range(len(structure_split)):
    if len(program_tower_indexes[i]) > 1:
        program_tower_errors[i] = check_tower_weights(i, program_tower_weights, program_tower_indexes)
for i in range(len(structure_split)):
    if program_tower_errors[i] != 0:
        print(i, program_tower_indexes[i][1:], [program_tower_weights[j] for j in program_tower_indexes[i][1:]])
print(program_weights[271]) # This is found from the previous output
