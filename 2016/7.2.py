...
IP_ADDR_LIST = "xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm,\
...
ssl_supported_count = 0
detected = 0
for ip_addr in IP_ADDR_LIST.split(','):
    ip_addr = ip_addr.replace(']', '[')
    words = ip_addr.split('[')
    for i in range(0, len(words), 2):
        for char in range(len(words[i]) - 2):
            if is_aba(words[i][char:char + 3]):
                for j in range(1, len(words), 2):
                    if words[j].find(words[i][char + 1:char + 3] + words[i][char + 1]) != -1:
                        ssl_supported_count += 1
                        detected = 1
                        break
                if detected == 1:
                    break
