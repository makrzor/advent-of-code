...
def decode(name, key):
    ...
    for letter in name:
        ...
        if code > 122:
            code -= 26
        name_decoded += chr(code)
    return name_decoded

sector_ids_sum = 0
letter_count = [0] * 124
for room in ROOMS_LIST.split(','):
    for asc in range(97, 123):
        letter_count[asc] = room[:-7].count(chr(asc))
    checksum = ""
    for i in range(5):
        max_count = max(letter_count)
        for asc in range(97, 123):
            if letter_count[asc] == max_count:
                letter_count[asc] = 0
                checksum += chr(asc)
                break
    if room[-6:-1] == checksum:
        print(decode(room[:-6 - len(checksum)], int(room[:-7].split('-')[-1])))
# now do | grep pole to get the answer
